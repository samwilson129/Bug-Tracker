...
public class TesterPanel extends VerticalLayout {
    ...
    private Component createReportBugContent() {
        ...
        submitButton.addClickListener(event -> {
            if (titleField.isEmpty() || projectField.isEmpty() || priorityField.isEmpty() || descriptionField.isEmpty()) {
            ...
            Bug newBug = new Bug(0, "", "", Bug.BugStatus.reported, Bug.Priority.Low,LocalDateTime.now(), LocalDateTime.now(), null, null, 0); 
    ...
    private Component createMyBugsContent() {
        ...
        long totalOpen = bugs.stream()
            .filter(b -> b.getter_bugstatus() == Bug.BugStatus.reported || b.getter_bugstatus() == Bug.BugStatus.in_progress)
            .count();
        long totalResolved = bugs.stream()
            .filter(b -> b.getter_bugstatus() == Bug.BugStatus.fixed || b.getter_bugstatus() == Bug.BugStatus.closed)
            .count();
        ...
        VerticalLayout reportedCard = createSummaryCard("Reported", String.valueOf(totalReported), VaadinIcon.BUG.create(), "#1676f3");
        VerticalLayout openCard = createSummaryCard("Open", String.valueOf(totalOpen), VaadinIcon.HOURGLASS.create(), "orange");
        VerticalLayout resolvedCard = createSummaryCard("Resolved", String.valueOf(totalResolved), VaadinIcon.CHECK.create(), "green");
